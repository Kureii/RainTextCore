...
static uint32_t scrypt_ilog2(uint32_t n) {
  ...
  while (((uint32_t)1 << t) < n) {
    if (t > SCRYPT_SAFE_N) return (uint32_t)-1; /* Check for insanity */
    t++;
  }
...
int libscrypt_mcf(uint32_t N, uint32_t r, uint32_t p, const char *salt,
   ...
   * struggle
   */
  if (r > (uint8_t)(-1) || p > (uint8_t)(-1)) return 0;
   ...
   * in length
   */
  s = SNPRINTF(mcf, SCRYPT_MCF_LEN, SCRYPT_MCF_ID "$%06x$%s$%s",
               (unsigned int)params, salt, hash);
  if (s >= SCRYPT_MCF_LEN) return 0;
