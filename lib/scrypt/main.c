...
*/
  ...
  printf(
      "TEST ONE: Direct call to reference function with password 'password' "
      "and salt 'NaCL'\n");

  retval = libscrypt_scrypt((uint8_t*)"password", strlen("password"),
                            (uint8_t*)"NaCl", strlen("NaCl"), 1024, 8, 16,
  ...
  retval = libscrypt_scrypt((uint8_t*)"password", strlen("password"),
                            (uint8_t*)"NaCl", strlen("NaCl"), 47, 1, 1, hashbuf,
                            sizeof(hashbuf));
  ...
  printf(
      "TEST FOUR: Direct call to reference function with pleaseletmein "
      "password and SodiumChloride as salt\n");

  /* Tests 4-6 repeat tests 1-3 with a different reference vector */

  retval =
      libscrypt_scrypt((uint8_t*)"pleaseletmein", strlen("pleaseletmein"),
                       (uint8_t*)"SodiumChloride", strlen("SodiumChloride"),
                       16384, 8, 1, hashbuf, sizeof(hashbuf));
...
* Note that this is not an exported function
  */
  ...
  retval =
      libscrypt_b64_encode((unsigned char*)"SodiumChloride",
                           strlen("SodiumChloride"), saltbuf, sizeof(saltbuf));
  if (retval == -1) {
    printf("TEST SEVEN FAILED\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (retval == 0) {
    printf(
        "TEST NINE: FAILED, claimed pleaseletmein hash claimed did not "
        "verify\n");
    exit(EXIT_FAILURE);
  }
  ...
  retval = libscrypt_b64_encode((uint8_t*)saltbuf, SCRYPT_SALT_LEN, outbuf,
                                sizeof(outbuf));
  if (retval == -1) {
  ...
  retval = libscrypt_hash(outbuf, "My cats's breath smells like cat food",
